...
const user={...newUser,reason,date}
const onChange = date =>{
  setDate(date);
};
...
[name]: value,
 ...
 <InputGroup size="sm" className="mb-3">
  ...
  <InputGroup size="sm" className="mb-3">
   ...
   <Calendar className='calendar'  onChange={onChange} name="date" value={newUser.date}  /></div>
   :null}
         </InputGroup>  
</Modal.Body>
              <Modal.Footer>
...
<div className="stat_out">    
                  <Button className="stat_btn" onClick={(e)=>{history.push("/stat")}} > 
               <IconContext.Provider value={{ color: "white", size:"2.5em" }}>
            < FcStatistics/> 
...
{(value==='All'?users:users.filter(el=>el.isDone===value)).map((user,id) =>
  ...
  <figcaption>
    ...
    <h3>Date</h3>
     ...
     <div className="icons">
     <IconContext.Provider value={{ color: "white", className: "edit", size:"2em" }}>
