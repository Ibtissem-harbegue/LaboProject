...
import { deleteUser, editUser } from "../../Redux/actions/authActions";
...
const AdminProfile = ({setSearch,users,logout,user}) => {
  ...
  const [result, setResult] = useState(user.status)
...
{(value==='All'?users:users.filter(el=>el.status===value)).map((user,id) =>
  ...
  <GoTrashcan  onClick={()=>dispatch(deleteUser(user._id))} />
      ...
      <Form.Control as="select"  onClick={handleConfirm} onChange={(e)=>{const x= e.target.value; setResult(x);setNewUser({...newUser,status:x})}} name="result">
